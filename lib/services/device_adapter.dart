...
abstract class DeviceAdapter {
  ...
  /// Set incline level (treadmill only, ignored by bikes and HR monitors)
  /// Level range depends on device type (typically -5 to 15 for most treadmills)
  Future<void> setIncline(double level);

  /// Get the configuration schema for this device
  /// Returns a map of configuration key -> ConfigurationItem
  Map<String, ConfigurationItem> getConfigurationSchema();

  /// Apply configuration values to the device
  /// config is a map of key -> value that should match the schema
